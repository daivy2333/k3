...
> 当前项目状态: `current`
> 最后同步: 2026-09-10 (Thu Sep 10 2026 14:39 GMT+0800) 增量刷新
> 同步者: `openspec-docs-maintainer`
> 同步 revision: `115f387` + 未提交 change `establish-k3-com260-dma-memory-ownership-baseline` + change `establish-k3-com260-gmac-network-baseline` 双 change 收尾同步 (MS06 + MS07 全部产品交付 + 项目级状态; 在 MS05 同步基线之上增量刷新)
...
- 工作区: change `establish-k3-doc-foundation` / `establish-k3-source-tracking-baseline` / `establish-k3-com260-board-boot-baseline` / `establish-k3-com260-platform-uart-baseline` / `establish-k3-com260-interrupt-time-notification-baseline` / `establish-k3-com260-dma-memory-ownership-baseline` / `establish-k3-com260-gmac-network-baseline` 已分别于 2026-09-05 / 2026-09-05 / 2026-09-08 / 2026-09-08 / 2026-09-09 / 2026-09-09 / 2026-09-10 收尾归档 (MS01 / MS02 / MS03 / MS04 / MS05 / MS06 / MS07); 工作区待常规 commit 的产品代码包括 `docs/index.md` / `docs/boot/com260-boot-chain.md` / `docs/boot/com260-image-and-dts.md` / `docs/platform/com260-board-resources.md` / `docs/platform/k3-soc-overview.md` / `docs/platform/k3-platform-control.md` / `docs/serial/com260-uart.md` / `docs/interrupts/k3-interrupt-and-time.md` / `docs/interrupts/com260-mailbox-notification.md` / `docs/dma/k3-dma-and-memory-ownership.md` / `docs/dma/k3-cache-pma-address-translation.md` / `docs/network/com260-gmac-phy.md` / `docs/network/k3-gmac-dma-irq.md` / `docs/reference/source-coverage.md` / `docs/reference/known-gaps.md` / `docs/reference/source-refresh.md` 与 `.claude/analysis/rt-async-amp-k3-{boot-platform,drivers,shared-memory,starryos-reuse}.md`。
- Git 分支: `main`
- 归档目录: `openspec/changes/archive/2026-09-05-establish-k3-doc-foundation/`, `openspec/changes/archive/2026-09-05-establish-k3-source-tracking-baseline/`, `openspec/changes/archive/2026-09-05-establish-k3-com260-board-boot-baseline/`, `openspec/changes/archive/2026-09-08-establish-k3-com260-platform-uart-baseline/`, `openspec/changes/archive/2026-09-09-establish-k3-com260-interrupt-time-notification-baseline/`, `openspec/changes/archive/2026-09-09-establish-k3-com260-dma-memory-ownership-baseline/`, `openspec/changes/archive/2026-09-10-establish-k3-com260-gmac-network-baseline/`
...
- `current`: 本次增量刷新同步了 change `establish-k3-com260-gmac-network-baseline` 收尾后的项目状态; MS07 已完成, 2 篇产品文档 (`docs/network/com260-gmac-phy.md` 238 行 / `docs/network/k3-gmac-dma-irq.md` 206 行, 均 < 450 行) + 70 URL 覆盖 (在 MS06 68 之上新增 K3 GMAC glue driver `dwmac-spacemit-ethqos.c` + 既有两篇 network 正文引用的官方 URL, URL 总数 69 → 70) + 10 个缺口 (G3 / G4 / G5 = `partial`, G6 / G7 = `open`, G1-G2 / G8-G10 状态保持) + delta spec 同步到 `openspec/specs/k3-com260-gmac-network-baseline/spec.md`（5 added, 0 removed, 0 modified） + R16 登记; Iter 000 完成 `com260-gmac-phy.md` 创建, Iter 000 / Cycle 001-rework 修复 PLAN-INVALID + ACT-DEVIATION 2 项 Important 收敛 accepted; Iter 001 完成 `k3-gmac-dma-irq.md` 创建 + G3–G7 收敛 + index 接入 (T5 修正 G4 汇总 `open → partial` 与 k3.dtsi 静态拓扑一致) + §8 DMA soft reset timeout (Acceptance 3) + U4 同步 reset → warning → 继续初始化语义 + index 维护规则计数 69 → 70 (Acceptance 5) 2 项 Plan Review Important 修复 (Plan Review 复审 accepted, 2 项 Minor 属 Act Response 历史表述精度不阻塞 Acceptance); 6/6 tasks 勾选, Persisted Evidence `none`, 未触动 `others/` 或 MS06 既有 staged 内容。MS06 已完成的产品交付 (2 篇 dma 文档 + 68 URL 覆盖 + G5 partial + delta spec + R15 登记) 在本次同步基线之上保留, 工作区 `main` 上 staged 未 commit 的双 change 产物 (MS06 + MS07) 仍待常规 commit。
