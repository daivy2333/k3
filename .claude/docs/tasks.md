...
> 当前状态: MS01-MS12 已完成, 无进行中任务, 无已承诺待办。
...
- Related changes: `establish-k3-general-peripheral-baseline` (2026-09-12 收尾, archived): 完成 3 篇产品文档 (`docs/peripherals/k3-gpio-pwm-ir.md` 159 行 / `docs/peripherals/k3-audio.md` 196 行 / `docs/peripherals/k3-wdt-rtc.md` 155 行, 均 < 450 行); 70 个唯一 URL 来源覆盖 (在 MS10 70 之上不增减, 6 个通用外设官网入口由 `deferred` 改为 `active`); 14 个缺口 (G1-G13 + G14, G3/G4/G5 partial 与 G7 open 保持, G14 新增承载通用外设板级映射 / 软件运行路径 / 恢复闭包); 10 项术语新增 (GPIO / PWM / pinmux / consumer / 波形 / IR-RX / I2S / SSPA / DAI / WDT / RTC / RPMI — 已并入 `terminology.md` 共 63 项主写法); delta spec 同步到 `openspec/specs/k3-general-peripheral-baseline/spec.md` (8 added, 0 removed, 0 modified); R25 登记; Iteration 000 / Cycle 000 完成 T1 (六个通用外设官网入口由 `deferred` 改为 `active` 职责, 复用既有 URL 行, 总数保持 70) + T1.2 (创建 `k3-gpio-pwm-ir.md` 159 行, 6 段 (1 范围 / 2 GPIO controller+pinctrl+IRQ / 3 PWM channel+pinmux+consumer / 4 IR-RX / 5 CoM260 板级映射 / 6 错误边界与未知项 U1-U5) + PWM 30/20/PWM0-PWM19 三种观察并列 + 两个 disabled IR-RX 节点 + CoM260 复用候选 + 8 个首行 URL 覆盖 + 5 个 U 字段); Iteration 001 / Cycle 000 完成 T2 (创建 `k3-audio.md` 196 行, 7 段 (1 范围 / 2 I2S/SSPA controller / 3 CPU DAI / 4 sound card / 5 codec/display endpoint / 6 DMA 与 power domain / 7 CoM260 板级映射 + 错误边界 + 未知项 U1-U4), AP `i2s0`-`i2s5` 完成路径限定 `&pdma`/APLIC source 72, codec endpoint 指向 `dp0/dp1`, `pdma1` source 150 明确未被本组节点引用; DMA/cache 通过 MS06 引用); Iteration 002 / Cycle 000 完成 T3 (创建 `k3-wdt-rtc.md` 155 行, 7 段 (1 范围 / 2 WDT / 3 MMIO RTC / 4 RPMI RTC / 5 生命周期与边界 / 6 错误恢复与状态机 / 7 CoM260 板级映射 + 未知项 U1-U4), WDT 实际 `spacemit-k1,wdt` compatible, TIMERS0 clock/reset, APLIC source 35 + restart 属性, MMIO RTC 与 RPMI RTC 分表记录, service 实现域和运行所有权保持未知); Iteration 003 / Cycle 000 完成 T4-T7 (G14 索引既有未知项 + 10 项术语 + index.md 三篇通用外设正文入口 + 70/63/14 计数 + `docs/peripherals/` 状态由 `待聚合；R06 状态 deferred` 改为 `已聚合(Iteration 000/001/002)；G7/G14 open`); 7/7 tasks 勾选, 末轮 Plan Review accepted, `Next Cycle: None` / `Next Iteration: None`, Persisted Evidence `none`, 未触动 `others/` 或既有 staged 内容, `git diff --check` 退出 0, `openspec validate --strict` 退出 0。
- Related references: R04, R05, R08, R25
...
- Related changes: `establish-k3-image-flashing-boot-operations-baseline` (2026-09-14 收尾, archived): 完成 3 篇产品文档 (`docs/boot/k3-image-build-and-artifacts.md` 134 行 / `docs/boot/k3-ram-boot-and-fastboot.md` 149 行 / `docs/boot/k3-flashing-and-recovery.md` 144 行, 均 < 450 行); 76 个唯一 URL 来源覆盖 (在 MS11 70 之上新增 6 个 K3-Ubuntu-Images 精确文件, URL 总数 70 → 76); 15 个缺口 (G1-G15, G3/G4/G5 partial 与 G7 open 保持, G15 新增承载 MS12 镜像/包/持久写入/下载/恢复闭包, 与 G7/G12 职责分离); 9 项术语新增 (BootROM / FSBL / ESOS / OpenSBI / FIT / Fastboot / Titan / GPT / MTD); delta spec 同步到 `openspec/specs/k3-image-flashing-boot-operations-baseline/spec.md` (8 added, 0 removed, 0 modified); R26 登记; 4 个 Iteration (000 sources-and-artifacts / 001 ram-boot / 002 persistent-recovery / 003 navigation) 各自 7/7 tasks 勾选, 末轮 Plan Review accepted, `Next Cycle: None` / `Next Iteration: None`, Persisted Evidence `none`, 未触动 `others/` 或既有 staged 内容, `git diff --check` 退出 0, `openspec validate --strict` 退出 0。
- Related references: R05, R07-R09, R12, R18, R20-R22, R26
...
- MS11 / change `establish-k3-general-peripheral-baseline` (2026-09-12 收尾, archived 于 2026-09-14): 完成 3 篇产品文档 (`docs/peripherals/k3-gpio-pwm-ir.md` 159 行 / `docs/peripherals/k3-audio.md` 196 行 / `docs/peripherals/k3-wdt-rtc.md` 155 行, 均 < 450 行); 70 个唯一 URL 来源覆盖 (在 MS10 70 之上不增减, 6 个通用外设官网入口由 `deferred` 改为 `active`); 14 个缺口 (G1-G13 + G14, G3/G4/G5 partial 与 G7 open 保持, G14 新增承载通用外设板级映射 / 软件运行路径 / 恢复闭包); 10 项术语新增 (GPIO / PWM / IR-RX / I²S / SSPA / DAI / WDT / RTC / RPMI / VCC_RTC); delta spec 同步到 `openspec/specs/k3-general-peripheral-baseline/spec.md` (8 added, 0 removed, 0 modified); R25 登记; Iteration 000 / Cycle 000 完成 T1 (六个通用外设官网入口由 `deferred` 改为 `active` 职责, 复用既有 URL 行, 总数保持 70) + T1.2 (创建 `k3-gpio-pwm-ir.md` 159 行, 6 段 (1 范围 / 2 GPIO controller+pinctrl+IRQ / 3 PWM channel+pinmux+consumer / 4 IR-RX / 5 CoM260 板级映射 / 6 错误边界与未知项 U1-U5) + PWM 30/20/PWM0-PWM19 三种观察并列 + 两个 disabled IR-RX 节点 + CoM260 复用候选 + 8 个首行 URL 覆盖 + 5 个 U 字段); Iteration 001 / Cycle 000 完成 T2 (创建 `k3-audio.md` 196 行, 7 段 (1 范围 / 2 I2S/SSPA controller / 3 CPU DAI / 4 sound card / 5 codec/display endpoint / 6 DMA 与 power domain / 7 CoM260 板级映射 + 错误边界 + 未知项 U1-U4), AP `i2s0`-`i2s5` 完成路径限定 `&pdma`/APLIC source 72, codec endpoint 指向 `dp0/dp1`, `pdma1` source 150 明确未被本组节点引用; DMA/cache 通过 MS06 引用); Iteration 002 / Cycle 000 完成 T3 (创建 `k3-wdt-rtc.md` 155 行, 7 段 (1 范围 / 2 WDT / 3 MMIO RTC / 4 RPMI RTC / 5 生命周期与边界 / 6 错误恢复与状态机 / 7 CoM260 板级映射 + 未知项 U1-U4), WDT 实际 `spacemit-k1,wdt` compatible, TIMERS0 clock/reset, APLIC source 35 + restart 属性, MMIO RTC 与 RPMI RTC 分表记录, service 实现域和运行所有权保持未知); Iteration 003 / Cycle 000 完成 T4-T7 (G14 板级映射 / 运行路径 / 恢复闭包 + 10 项术语 + index.md 三篇通用外设正文入口 + 70/63/14 计数 + `docs/peripherals/` 状态由 `待聚合；R06 状态 deferred` 改为 `已聚合(Iteration 000/001/002)；G7/G14 open`); 7/7 tasks 勾选, 末轮 Plan Review accepted, `Next Cycle: None` / `Next Iteration: None`, Persisted Evidence `none`, 未触动 `others/` 或既有 staged 内容, `git diff --check` 退出 0, `openspec validate --strict` 退出 0。
- MS12 / change `establish-k3-image-flashing-boot-operations-baseline` (2026-09-14 收尾, archived): 完成 3 篇产品文档 (`docs/boot/k3-image-build-and-artifacts.md` 134 行 / `docs/boot/k3-ram-boot-and-fastboot.md` 149 行 / `docs/boot/k3-flashing-and-recovery.md` 144 行, 均 < 450 行); 76 个唯一 URL 来源覆盖 (在 MS11 70 之上新增 6 个 K3-Ubuntu-Images 精确文件, URL 总数 70 → 76); 15 个缺口 (G1-G15, G3/G4/G5 partial 与 G7 open 保持, G15 新增承载镜像/包/持久写入/download/恢复闭包, 与 G7/G12 职责分离); 9 项术语新增 (BootROM / FSBL / ESOS / OpenSBI / FIT / Fastboot / Titan / GPT / MTD); delta spec 同步到 `openspec/specs/k3-image-flashing-boot-operations-baseline/spec.md` (8 added, 0 removed, 0 modified); R26 登记; 4 个 Iteration 均 completed, 7/7 tasks 勾选, 末轮 Plan Review accepted, `Next Cycle: None` / `Next Iteration: None`, Persisted Evidence `none`, 未触动 `others/` 或既有 staged 内容, `git diff --check` 退出 0, `openspec validate --strict` 退出 0。
